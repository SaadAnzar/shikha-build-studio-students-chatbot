...
export default function StudentChatbotPage({ id }: StudentChatbotPageProps) {
  ...
  const rateConversation = async () => {
    if (chats.length < 5) {
      ...
      let inputRating = ""
      if (rubric) {
        inputRating = `Rubric for evaluating a student on a conversation is as follows:
...
Please rate the student on conversation based on the rubric. Give the output always in the following format only:
{
